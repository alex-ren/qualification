...
question about whether the implementation process or process derived from it is a
refinement of the
...
In ``classical'' LTL model checking, that a system satisfies an LTL formula fi
means that all of its computations satisfy fi and a computation is defined as an
infinite sequence
of states, the transitions of which are possbile in the system. Similar concept is applied to
CSP process in this paper. That a process satisfies an LTL formula fi means that
all of its traces satisfy fi. And a trace is an infinite\footnote todo sequence of events that
the process can possibly communicate with. Given an infinite trace ...

that a computation satisfies an LTL formula
means 


correctness properties
are encoded as processes as well, which we call specification processes,specified in the same language as the systems to be checked. 

correctness properties are usually encoded in
certain formal languages. Such properties are then checked against
CSP offers a unique way of verification of correctness properties of finite

This paper studies the possiblility of doing LTL model checking on CSP
specification in the context of refinement.

Def. Trace V.S. LTL

Problem with refinement checking

paper's method

refinement and LTL properties.
